...
def check_input_files(files_path, marker_id, flag='txt'):
    if flag == 'txt':
        ...
        samples_nums = len(os.listdir(unzip_path)) / 2
        txt_files = [fl for fl in os.listdir(unzip_path) if fl.endswith('.txt')]
        for txt_file in txt_files:
            check_report += check_file(os.path.join(unzip_path, txt_file), txt_file, flag) + '\n'
        if not check_report:
