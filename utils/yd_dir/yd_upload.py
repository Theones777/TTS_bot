...
from utils.variables import AVAIL_TXT_PROJECTS_NAMES, YD_HARD_HOMOGRAPHS_PATH, YD_DONE_NOT_ACCEPTED_HOMOGRAPHS_PATH, \
    ...
    YD_ROOT_DICTORS_CUTTED_AUDIOS_PATH, YD_ROOT_DICTORS_DONE_AUDIOS_PATH, YD_ROOT_DICTORS_RESERVE_AUDIOS_PATH
from utils.yd_dir.yd_init import y_disk


def move_file_to_yd(message, project_name, file_name, sample, flag, marker_id, mark_done, prev_marker_id):
    if flag:
    ...
    # else:
    #     yd_path = os.path.join(YD_PATH_MNT, project_name, mark_done)
    #     try:
    #         if file_name in os.listdir(os.path.join(YD_PATH_MNT, project_name, mark_done)):
    #             os.remove(os.path.join(TMP_DOWNLOAD_PATH, marker_id, file_name))
    #             return f'Файл с таким именем ({file_name}) уже загружен'
    #     except FileNotFoundError:
    #         os.remove(os.path.join(TMP_DOWNLOAD_PATH, marker_id, file_name))
    #         return 'Нужная папка не найдена (проверьте корректность выбора проекта)'
    #     shutil.move(os.path.join(TMP_DOWNLOAD_PATH, marker_id, file_name),
    #                 os.path.join(yd_path, f'{marker_id}_{sample}_{mark_done}.txt'))
    #
    #     str_to_return = f'Файл {file_name} перемещен в {project_name}'
        return str_to_return


def upload_to_yd(project_name, download_file_path, file_name, flag='sound_man'):
    out_str = ''
    if project_name in AVAIL_TXT_PROJECTS_NAMES[:2]:
        ...
        if flag == 'sound_man':
            dones_path = f'{YD_ROOT_DICTORS_CUTTED_AUDIOS_PATH}/{file_name}'
        else:
